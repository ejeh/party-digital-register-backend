...
//   private validateGeographicalScope(createElectionDto: CreateElectionDto) {
//     const { type, level, state, lga, ward } = createElectionDto;
...
//         // Allow both LGA-wide and Ward-level
//         if (level === 'lga') {
//           if (!lga) {
//             throw new BadRequestException(
//               'LGA is required for LGA-level elections',
//             );
//           }
//         } else if (level === 'ward') {
//           if (!lga || !ward) {
//             throw new BadRequestException(
//               'LGA and Ward are required for Ward-level elections',
//             );
//           }
//         } else {
//           throw new BadRequestException(
//             'Local government elections must be at LGA or Ward level',
//           );
//         }
//         break;
...
//         if (state || lga || ward) {
//           throw new BadRequestException(
//             'Geographical scope (state, LGA, ward) should not be specified for national elections',
//           );
//         }
//         if (level !== 'national') {
//           throw new BadRequestException(
...
//     // Extra redundancy guards
//     if (level === 'state' && !state) {
//       throw new BadRequestException(
//         'State is required for state-level elections',
//       );
//     }
//     if (level === 'lga' && (!state || !lga)) {
//       throw new BadRequestException(
//         'State and LGA are required for LGA-level elections',
//       );
//     }
//     if (level === 'ward' && (!state || !lga || !ward)) {
//       throw new BadRequestException(
//         'State, LGA and Ward are required for Ward-level elections',
//       );
//     }
//   }
...
//     const results = await this.getResults(electionId, level, locationId);
...
//     // Check if party already has a candidate for this position
//     const partyCandidateExist = await this.candidateModel.findOne({
//       electionId: new Types.ObjectId(electionId),
//       position: normalizedPosition,
//       party: createCandidateDto.party.toUpperCase(),
...
//       .findById(candidate._id)
//       .populate('userId', 'firstname lastname email phoneNumber')
//       .populate('electionId', 'title startDate endDate level state lga ward');
//   }
...
//     const { start, length, search, order, columns, draw, position } = params;
